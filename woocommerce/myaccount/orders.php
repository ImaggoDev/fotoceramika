...
<div class="orders">
<h3 class="title">Twoje zamówienia</h3>	
					...
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-date' === $column_id ) : ?>
								<time datetime="<?php echo esc_attr( $order->get_date_created()->date( 'd' ) ); ?>"><?php echo esc_html( $order->get_date_created()->date( 'd-m-Y' ) ); ?></time>
								...
								if ( version_compare( WC_VERSION, '3.2', '>=' ) ) {
									...
									if ( ! empty( $actions ) ) {
										foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited
											echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button button wc-action-btn ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '<i class="icon icon_btn_arrow"></i></a>';
										}
									}
								} else {
									$actions = array(
